...
def register() :
    ...
    # 지도만들기
    if(os.path.isfile("./templates/map.html") == False) :
        ...
        file = open('./templates/map.html')
        m = folium.Map(file)
        add_name = doc['name']
        add_address = doc['address']
        xy_code = CodinateMake(add_address)
        add_x_code = xy_code[0]
        add_y_code = xy_code[1]
        add_visit = doc['visit']
        if add_visit == "visit_ok":
            folium.Marker(location=[add_y_code, add_x_code], tooltip=add_name, icon=folium.Icon(color="blue", icon="home")).add_to(
                m)
        else:
            folium.Marker(location=[add_y_code, add_x_code], tooltip=add_name, icon=folium.Icon(color="red", icon="star")).add_to(m)
        m.save('./templates/map.html')
        print('지도화 성공!')
        return jsonify({'result': 'success'})
